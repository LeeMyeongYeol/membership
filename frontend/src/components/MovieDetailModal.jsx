...
function MovieDetailModal({ movie, isOpen, onClose }) {
  ...
  const fetchStreaming = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/streaming/${movie.id}?region=KR`)
              ...
              {reviewStats && (
                <div className="review-stats">
                  ...
                  {reviewStats.total_reviews > 0 && (
                    <span> | 평균 ★ {reviewStats.average_rating.toFixed(1)}</span>
                  )}
                </div>
              )}
                ...
                {loadingReviews ? (
                  ...
                  reviews.map((review) => (
                    <div key={review.id} className="review-item">
                      ...
                      <span className="review-date">{new Date(review.created_at).toLocaleDateString('ko-KR')}</span>
                    </div>
                  ))
                )}
              </div>
