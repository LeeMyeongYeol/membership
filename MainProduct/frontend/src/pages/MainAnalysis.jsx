...
function MainAnalysis() {
  ...
  const renderMovieCard = (item) => {
    const poster = item.poster ? (
      ...
      <div
        onClick={() => handleMovieClick(item)}
        style={{ height: '180px', background: '#0b1430', cursor: 'pointer' }}
      ></div>
    );
    ...
    const extra = [
      ...
      item.vote_average
        ? `TMDb ★${item.vote_average.toFixed(1)} (${item.vote_count || 0})`
        : null,
    ...
    <div className="main-analysis">
      <header className="page-header">
        <div className="container">
          <h1> 영화 취향 분석 추천 </h1>
          <div className="hint">
            TMDb + OMDb + Python TF‑IDF로 내가 좋아할 영화를 찾아줍니다.
          </div>
        </div>
      </header>
      ...
      <div className="container">
        <div className="grid">
          <div className="card">
            ...
            <p className="hint" style={{ marginTop: '10px' }}>
              * 서버의 .env에 TMDB_API_KEY, OMDB_API_KEY를 넣어두면 자동
              사용합니다.
            </p>
          </div>
        ...
        <div className="card" style={{ marginTop: '20px' }}>
          <h3>3) 맞춤 추천</h3>
          {error && (
            <div className="hint" style={{ color: '#ff9c9c' }}>
              {error}
            </div>
          )}
          {result && (
      ...
      <footer className="page-footer">
        © TF‑IDF Recommender • TMDb/OMDb 데이터를 사용합니다.
      </footer>
